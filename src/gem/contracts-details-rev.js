 ...
 *   node src/gem/contracts-details-rev.js --reverse
 *   node src/gem/contracts-details-rev.js --delay-3
 *   node src/gem/contracts-details-rev.js --name "Autonomous Body" --delay-3
 *   node src/gem/contracts-details-rev.js --parts=10 --part=1 --delay-3
 *   node src/gem/contracts-details-rev.js --from 01-01-2021 --to 31-12-2021
 */
...
/** Same 90-day windows as the forward scanner (fromDate → toDate = +90, then next = toDate+1). */
function buildDateWindows(startDay, endDay) {
  const windows = [];
  let day = startDay;
...
async function scanMinistry({
  ...
  // Walk the same windows as the forward script, newest first (end → start).
  const windows = buildDateWindows(startDay, endDay).reverse();
  let savedCount = 0;
  let skippedEmpty = 0;
  let skippedDup = 0;
  ...
  for (let i = 0; i < windows.length; i++) {
    ...
    const nextLabel = nextWindow
      ? `${formatShort(nextWindow.fromDate)} to ${formatShort(nextWindow.toDate)}`
      : 'done';
    ...
    if (totalPages === 0) {
      console.log('  no data → prevdate', nextLabel);
      skippedEmpty += 1;
      continue;
    }
    ...
    console.log('  prevdate:', nextLabel);
  }
...
async function main() {
  ...
  console.log(
    `date scan: ${formatShort(endDay)} → ${formatShort(startDay)} (+90 day windows, reverse)`
  );
