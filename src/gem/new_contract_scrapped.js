...
async function enrichStateContract({ client, s3, row, cookie, delayMs, block }) {
  ...
  if (!pdfUrl) {
    ...
    console.log(`      stored PDF URL exists`);
    try {
      pdfBuf = await downloadPdfFromUrl(pdfUrl, delayMs);
    } catch (err) {
      console.log(`      stored pdf failed (${err.message}) — fallback GeM`);
      try {
        pdfBuf = await downloadPdf(orderId, delayMs);
      } catch (gemErr) {
        throw stageError('pdf', gemErr);
      }
      try {
        pdfUrl = await uploadPdfToS3(s3, pdfBuf, contractNumber);
        await updateNewContractOrderAndPdf(client, row.id, orderId, pdfUrl);
        console.log(`      pdf uploaded to S3 (GeM fallback)`);
      } catch (s3Err) {
        throw stageError('s3 upload', s3Err);
      }
    }
  }
  ...
  let parsed;
  try {
    parsed = parsePdfSections(await extractPdfText(pdfBuf));
  } catch (err) {
