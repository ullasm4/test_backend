...
exports.controller = async (req, res, _next, db) => {
  if (!req.user?.id) {
    throw new ServerError('Login required to view WhatsApp logs', 401, ErrorCode.UNAUTHORIZED);
  }
  ...
  const [countRes, rowsRes] = await Promise.all([
    db.query('SELECT COUNT(*)::int AS total FROM seller_whatsapp_log l WHERE l.seller_id = $1', [sellerId]),
    db.query(
      `
      SELECT
        l.id,
