...
-- Lookup table for Indian cities (linked to existing states).
-- No created_by. Uses INNER JOIN so unknown state names are skipped (no NULL state_id).
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_cities_name_state_id ON cities (name, state_id);
CREATE INDEX IF NOT EXISTS idx_cities_state_id ON cities (state_id);
CREATE INDEX IF NOT EXISTS idx_cities_name_lower ON cities (lower(btrim(name)));

-- Insert only rows whose state name exists in public.states (avoids NULL state_id).
INSERT INTO cities (name, state_id)
SELECT v.city_name, s.id
FROM (
  VALUES
  ...
  ('Adra', 'West Bengal')
) AS v(city_name, state_name)
INNER JOIN states s ON lower(btrim(s.name)) = lower(btrim(v.state_name))
...
DROP INDEX IF EXISTS uk_cities_name_state_id;
